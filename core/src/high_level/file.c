...
void* high_level__file_get(const char * name, int* data_length, int* error_code) {
    ...
    found_file_info_t found_file = dir_data_id__get_file_in_subdirs(ROOT_DIR_ID, name2, 0, error_code);
    if (*error_code != 0) {
        return NULL;
    }
    ...
    file_id__get_contents(found_file.file_i_node_id, (uint8_t *) contents, error_code);
    if (*error_code != 0) {
        return NULL;
    }
...
void high_level__file_copy(char * source, char * destination, int * error) {
    ...
    high_level__file_write(destination, contents, file_length, error);
    if (*error != 0) {
