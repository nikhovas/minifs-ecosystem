...
void i_node__add_blocks_to_end(struct minifs_core__filesystem_context * ctx, i_node_data_t *i_node_data, uint8_t new_blocks, int* error) {
    *error = NO_ERROR;
    ...
    const uint8_t block_count = i_node__get_block_count(ctx, i_node_data);
    for (uint8_t i = block_count; i < block_count + new_blocks; ++i) {
        i_node_data->block_ids[i] = alloc_block(ctx, error);
        if (*error != NO_ERROR) {
            for (int16_t j = i; j >= block_count; --j) {
                free_block(ctx, i_node_data->block_ids[j], error);
                if (*error != NO_ERROR) {
                    return;
                }
...
void i_node__set_block_count(struct minifs_core__filesystem_context * ctx, i_node_data_t *i_node_data, uint8_t new_block_size, int * error) {
    *error = NO_ERROR;
    ...
    if (block_number > new_block_size) {
        i_node__remove_blocks_from_end(ctx, i_node_data, block_number - new_block_size, error);
    } else if (block_number < new_block_size) {
        i_node__add_blocks_to_end(ctx, i_node_data, new_block_size - block_number, error);
    }
}