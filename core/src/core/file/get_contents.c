...
void file__get_contents(struct minifs_core__filesystem_context * ctx, i_node_data_t *i_node_data, uint8_t* contents, int *error) {
    i_node_block_info_t block_info = get_i_node_block_info(ctx, i_node_data);

    uint8_t i = 0;
    for (; i < block_info.number_of_blocks - 1; ++i) {
        block_data_t block_data = file__get_block_data(ctx, i_node_data, i, error);
        if (*error != NO_ERROR) {
            return;
        }
    ...
    block_data_t block_data = file__get_block_data(ctx, i_node_data, block_info.number_of_blocks - 1, error);
    if (*error != NO_ERROR) {
        return;
    }
...
void file_id__get_contents(struct minifs_core__filesystem_context * ctx, uint8_t i_node_id, uint8_t* contents, int *error) {
    ...
    file__get_contents(ctx, &i_node_data, contents, error);
    if (*error != NO_ERROR) {
        return;
    }
}
