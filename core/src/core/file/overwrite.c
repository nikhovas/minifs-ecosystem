...
void file__overwrite(struct minifs_core__filesystem_context * ctx, i_node_data_t *i_node_data, uint8_t* data, uint16_t length, int * error) {
    *error = NO_ERROR;
    ...
    i_node__set_block_count(ctx, i_node_data, new_block_number, error);
    if (*error != NO_ERROR) {
        return;
    }
    ...
    for (uint8_t i = 0; i < new_block_number - 1; ++i) {
        ...
        file__set_block_data(ctx, i_node_data, i, &block_data, error);
        if (*error != NO_ERROR) {
            return;
        }
    }
    ...
    file__set_block_data(ctx, i_node_data, new_block_number - 1, &block_data, error);
    if (*error != NO_ERROR) {
        return;
    }
}
...
void file_id__overwrite(struct minifs_core__filesystem_context * ctx, uint8_t i_node_id, uint8_t* data, uint16_t length, int * error) {
    i_node_data_t file_i_node_data = i_node_id__get_data(ctx, i_node_id, error);
    if (*error != NO_ERROR) {
        return;
    }

    file__overwrite(ctx, &file_i_node_data, data, length, error);
    if (*error == NO_ERROR) {
        i_node__save(ctx, i_node_id, &file_i_node_data, error);
        if (*error != NO_ERROR) {
            return;
        }
    }
